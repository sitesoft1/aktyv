...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `roz_status` = '" . (isset($data['roz_status']) ? (int)$data['roz_status'] : 0) . "', `roz_footwear` = '" . (isset($data['roz_footwear']) ? (int)$data['roz_footwear'] : 0) . "', `column` = '" . (int)$data['column'] . "', `roz_category_id` = '" . (int)$data['roz_category_id'] . "', `roz_ratio` = '" . (float)$data['roz_ratio'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function editCategory($category_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `roz_status` = '" . (isset($data['roz_status']) ? (int)$data['roz_status'] : 0) . "', `roz_footwear` = '" . (isset($data['roz_footwear']) ? (int)$data['roz_footwear'] : 0) . "', `column` = '" . (int)$data['column'] . "', `roz_category_id` = '" . (int)$data['roz_category_id'] . "', `roz_ratio` = '" . (float)$data['roz_ratio'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategoryDescriptions($category_id) {
		...
		foreach ($query->rows as $result) {
			$category_description_data[$result['language_id']] = array(
                ...
                'roz_description'      => $result['roz_description']
			);
		}
